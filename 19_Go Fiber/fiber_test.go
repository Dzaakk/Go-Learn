...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=dz")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"dz", "password":"rahasia"}`)
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"dzk", "password":"rahasia", "name":"dzk"}`)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success dzk", string(bytes))
}

func TestBodyParserFORM(t *testing.T) {
	TestBodyParser(t)

	body := strings.NewReader(`username=dzk&password=rahasia&name=dzk+m`)
	request := httptest.NewRequest("POST", "/register", body)
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	response, err := app.Test(request)
	assert.Nil(t, err)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"yato","username":"yatoyato"}`, string(bytes))
}
...
func TestView(t *testing.T) {
	...
	assert.Contains(t, string(bytes), "HEADER")
	assert.Contains(t, string(bytes), "CONTENT")
}
