...
func TestJson(t *testing.T) {
	...
	assert.Equal(t, "learn-viper", config.GetString("app.name"))
	assert.Equal(t, "Dz", config.GetString("app.author"))
	assert.Equal(t, "localhost", config.GetString("database.host"))
	assert.Equal(t, "3306", config.GetString("database.port"))
...
func TestEnv(t *testing.T) {
	...
	assert.Equal(t, "learn-viper", config.GetString("APP_NAME"))
	assert.Equal(t, "Dz", config.GetString("APP_AUTHOR"))
...
func TestENV(t *testing.T) {
	...
	assert.Equal(t, 3306, config.GetInt("DATABASE_PORT"))
	assert.Equal(t, true, config.GetBool("DATABASE_SHOW_SQL"))
	assert.Equal(t, "Hello", config.GetString("KEY_ENV"))
}
